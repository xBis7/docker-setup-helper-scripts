...
# Hive jars names
HIVE_BEELINE_JAR_NAME="hive-beeline-4.0.0.jar"
HIVE_CLI_JAR_NAME="hive-cli-4.0.0.jar"
HIVE_COMMON_JAR_NAME="hive-common-4.0.0.jar"
HIVE_EXEC_CORE_JAR_NAME="hive-exec-4.0.0-core.jar"
HIVE_EXEC_JAR_NAME="hive-exec-4.0.0.jar"
HIVE_JDBC_STANDALONE_JAR_NAME="hive-jdbc-4.0.0-standalone.jar"
HIVE_JDBC_JAR_NAME="hive-jdbc-4.0.0.jar"
HIVE_LLAP_COMMON_JAR_NAME="hive-llap-common-4.0.0.jar"
HIVE_METASTORE_JAR_NAME="hive-metastore-4.0.0.jar"
HIVE_SERDE_JAR_NAME="hive-serde-4.0.0.jar"
HIVE_SERVICE_RPC_JAR_NAME="hive-service-rpc-4.0.0.jar"
HIVE_SHIMS_JAR_NAME="hive-shims-4.0.0.jar"
HIVE_SHIMS_COMMON_JAR_NAME="hive-shims-common-4.0.0.jar"
HIVE_SHIMS_SCHEDULER_JAR_NAME="hive-shims-scheduler-3.1.3.jar"
HIVE_SPARK_CLIENT_JAR_NAME="hive-spark-client-3.1.3.jar"
HIVE_STANDALONE_METASTORE_SERVER_JAR_NAME="hive-standalone-metastore-server-4.0.0.jar"
HIVE_STANDALONE_METASTORE_BENCHMARKS_JAR_NAME="hive-metastore-benchmarks-4.0.0.jar"
HIVE_STANDALONE_METASTORE_TOOLS_COMMON_JAR_NAME="metastore-tools-common-4.0.0.jar"

# We probably don't need those. Don't copy them for now. They are both under 'ql/target'
HIVE_EXEC_FALLBACKAUTHORIZER_JAR_NAME="hive-exec-4.0.0-fallbackauthorizer.jar"
HIVE_ORIGINAL_EXEC_JAR_NAME="original-hive-exec-4.0.0.jar"
...
handleHiveEnv() {
  ...
  hive_docker_path="$abs_path/$PROJECT_HIVE/packaging/target/apache-hive-4.0.0-bin/apache-hive-4.0.0-bin/compose/hive-metastore-ranger"
  cd $hive_docker_path
...
handleTrinoEnv() {
  ...
  trino_docker_path="$abs_path/docker-setup-helper-scripts/compose/trino"
  ...
  if [ "$op" == "start" ]; then
    ...
    docker compose -f "$trino_docker_path/docker-compose.yml" up -d
    ...
    docker compose -f "$trino_docker_path/docker-compose.yml" down
...
handleSparkEnv() {
  ...
  spark_docker_path="$abs_path/$PROJECT_SPARK/dist/compose/spark"
  cd $spark_docker_path
  ...
  if [ "$op" == "start" ]; then
    ...
    docker compose -f "$spark_docker_path/docker-compose.yml" down
