...
sql="spark.sql(\\\"create table $TABLE_ANIMALS (id int, name string) using parquet partitioned by (name)\\\")"
...
sql="spark.sql(\\\"alter table $TABLE_ANIMALS add partition (name='cow')\\\")"
retryOperationIfNeeded "$abs_path" "performSparkSql $sql" "$successMsg" "false"
...
echo "- INFO: [create] should succeed"
successMsg="org.apache.spark.sql.DataFrame = []"
sql="spark.sql(\\\"create table $TABLE_SPORTS (id int, name string)\\\")"
retryOperationIfNeeded "$abs_path" "performSparkSql $sql" "$successMsg" "false"
